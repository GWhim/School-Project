<template>
    <div id="xqtm">
        ...
        <el-row class="xqtm-center">
            ...
            <el-col :span="18" class="xqtm-center-right">
                <el-col :span="6"class="xqtm-select-item2">
                 ...
                 <el-table :data="xqtmData" height="500" empty-text="暂无需求文件"class="xqtm-table2" >
                    <el-table-column  align="center" prop="id" label="id"  width="0" v-if="false" sortable> </el-table-column>
                     ...
                     <el-table-column align="center"  prop="doneworknum" label="状态"  width="120"  sortable> 
                         <template slot-scope="scope">
                           <span v-if="scope.row.status==0"style="color:	#DC143C" >已关闭</span>
                            ...
                            <span v-if="scope.row.status==2">已验证</span>
                         </template>
                     </el-table-column>
                    <el-table-column align="center"  label="操作" width="300">
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="(user.identity==1||user.identity==2)&&scope.row.status==1"@click="xqtmdone(scope.row)" type="text" size="small">验证完成</el-button>
                        </template>
                     </el-table-column>
                </el-table>
            </el-col>
        ...
        <el-dialog :visible.sync="dialogFormVisible" customClass="cyglAdd"   width="30%"center>
            <template slot="title">
              <div style="text-align: center; color: #fff; font-size: 20px;">向项目[ {{project}} ]添加新需求条目</div>    
            </template>
          <el-row style="height:280px">
              <el-form :model="Form" :rules="rules" ref="xqtmruleForm" label-width="100px" class="demo-ruleForm">
                   <el-form-item label="名称" prop="xqtmtitle">
                        <el-input placeholder="请输入需求标题"v-model="Form.xqtmtitle"></el-input>
                    </el-form-item>
              <div style="padding-top:20px"></div>
              <el-form-item label="描述" prop="textarea">
                <el-input type="textarea":rows="10"placeholder="请输入需求描述"v-model="Form.textarea"maxlength="1000"show-word-limit></el-input>
               </el-form-item>
              </el-form>
          </el-row>
          <div slot="footer" class="dialog-footer">
            <el-button type="primary" @click="submitForm('xqtmruleForm')" :loading="logining">确 定</el-button>
          </div>
        </el-dialog>
...
export default {
      ...
      return{
          ...
          user:{},
           ...
           Form:{
            ...
            rules: { xqtmtitle: [{ required: true, message: '请输入标题', trigger: 'blur' },],
 ...
 methods:{
    ...
    allCurrentChange(val){
          var _this=this;
          _this.demandId=val.demandId;
        this.$axios.post("/user/getDemandItemBydemandId",{demandId:val.demandId}).then(resp => {
            if (resp ) {
              console.log(resp)
              if(resp.data.code!=200){
                  this.$message({"message":resp.data.msg,center: true,showClose:true,type:'error',duration:800});
    ...
    submitForm(val){
         ...
         this.$refs[val].validate((valid) => {
          if (valid) {
         _this.logining=true;
                this.$axios.post("/user/addxqtm",{uId:_this.user.id,textarea:_this.Form.textarea,xqtmtitle:_this.Form.xqtmtitle,vId:_this.value,demandId:_this.demandId}).then(resp => {
                if (resp ) {
                console.log(resp)
                if(resp.data.code!=200){
                    this.$message({"message":resp.data.msg,center: true,showClose:true,type:'error',duration:800});
                    ...
                    this.$message({"message":resp.data.msg,center: true,showClose:true,type:'success',duration:2000});
                     ...
                     _this.allCurrentChange({'demandId':_this.demandId});
                    }
                }
                })
          }
    ...
    xqtmdetil(val){ 
        ...
        if(val.status==0){
             ...
             _this.rwxq+="<br/><br/>"+"验证人："+val.enduser+" 验证时间："+val.endtime;
        }
    ...
    xqtmclose(val){
        ...
        this.$confirm('确认关闭？')
          .then(_ => {
            this.$axios.post("/user/updatexqtmstatus",{userId:_this.user.id,xqtmId:val.id,status:"0"}).then(resp => {
                if (resp ) {
                if(resp.data.code!=200){
                    this.$message({"message":resp.data.msg,center: true,showClose:true,type:'error',duration:800});
                }else{
    ...
    xqtmdone(val){
        ...
        this.$confirm('验证完成？')
          .then(_ => {
            this.$axios.post("/user/updatexqtmstatus",{userId:_this.user.id,xqtmId:val.id,status:"2"}).then(resp => {
                if (resp ) {
                if(resp.data.code!=200){
                    this.$message({"message":resp.data.msg,center: true,showClose:true,type:'error',duration:800});
                }else{
