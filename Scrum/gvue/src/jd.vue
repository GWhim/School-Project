<template>
    <div id="jd">
        <router-view></router-view>
         <el-row class="jd-top">
            ...
            <el-date-picker  v-model="timevalue" type="daterange"@input="projectChange(2)"align="center" start-placeholder="任务创建开始日期"end-placeholder="任务创建结束日期"  range-separator="——":picker-options="pickerOptions"></el-date-picker>
             <el-col :span="6" class="">
             <el-button v-if="qx" @click="addRw()" type=""class="jd-addRw">增加任务</el-button>
             </el-col>
             </el-row>
        </el-row>
        <el-row class="jd-top-center">
             <el-row style="height:100%">
                    <div style="position: absolute;">任务进度</div>
                    <el-row class="jd-main" v-if="projectSelectValue!=''">
                    <el-col :span="4" v-for="(item,i) in vOptions" :key="item.value" class="progress-item">
                        <el-row class="progress-item2">
                             <el-col class="progress-item">
                             <el-progress type="dashboard" :percentage="item.worknum" :color="colors"></el-progress>
                               <div class="progress-text" @click="projectChange(9,item)">{{item.label}}</div>
                             </el-col>
                        <div class="next" v-if="i!=vOptions.length-1">==></div>
                        </el-row>
                       
                    </el-col>
                    </el-row>
                    <el-row class="jd-main-text" v-if="projectSelectValue==''">
                    <div style="color:#909399 ;">还未选择项目/迭代</div>
                    </el-row>
            </el-row>
        ...
        <el-row class="jd-center">
           <el-row>
               <el-table :data="rWData" height="450" empty-text="暂无任务"class="rw-table" :header-cell-style="{'border-bottom': '1px solid #2197F1'}">
        ...
        <el-dialog :visible.sync="addRwDialog" customClass="global-bk"   width="30%"center>
             <template slot="title">
              <div style="text-align: center; color: #fff; font-size: 20px;">添加任务</div>    
            </template>
            <el-form :model="form" ref="mmForm" >
                ...
                <el-form-item label="选择任务指派人" label-width="150px" prop="selectUser">
...
export default {
 ...
 methods:{
    ...
    submitaddRw(){
        var _this=this;
        this.$axios.post("/user/addRw",{createUserId:_this.user.id,vId:_this.form.addRwVValue,selectUser:_this.form.selectUser,des:_this.form.des}).then(resp => {
            if (resp ) {
              console.log(resp)
              if(resp.data.code!=200){
                  this.$message({"message":resp.data.msg,center: true,showClose:true,type:'error',duration:800});
    ...
    toDo(val,a){
     ...
     dorw(val){
        this.$axios.post("/user/updateWorkStauts",{workId:val.id,userId:this.user.id}).then(resp => {
            if (resp ) {
              console.log(resp)
              if(resp.data.code!=200){
                  this.$message({"message":resp.data.msg,center: true,showClose:true,type:'error',duration:800});
    ...
    toUser(){
        var _this=this;
        this.$axios.post("/user/zjrw",{loginUserId:_this.user.id,toUserId:_this.ortherUserValue,workId:_this.valtemp.id}).then(resp => {
            if (resp ) {
              if(resp.data.code!=200){
                  this.$message({"message":resp.data.msg,center: true,showClose:true,type:'error',duration:800});
              }else{
    ...
    doclose(val){
        var _this=this;
        this.$axios.post("/user/closerw",{loginUserId:_this.user.id,workId:_this.valtemp.id}).then(resp => {
            if (resp ) {
              if(resp.data.code!=200){
                  this.$message({"message":resp.data.msg,center: true,showClose:true,type:'error',duration:800});
              }else{
